...
namespace Power_Station_System.TabControlZ
{
    class TabControlZ : System.Windows.Forms.TabControl
    {
        ...
        Color c2 = Color.FromArgb(color2Transparent, nonactive_color2);
            ...
            Rectangle rrr = new Rectangle(e.Bounds.Right - 15, e.Bounds.Top + 5, 12, 12);
            using (LinearGradientBrush br = new LinearGradientBrush(rc, Color.Transparent, Color.Transparent , angle))
            {
                e.Graphics.FillRectangle(br, rrr);
            }
            e.Graphics.DrawString("×", new Font("Arial", 12, FontStyle.Bold), Brushes.Snow, e.Bounds.Right + 1 - 15, e.Bounds.Top + 4);
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            for (int i = 0; i < this.TabPages.Count; i++)
            {
                ...
                {
                    if (MessageBox.Show("هل تريد إغلاق هذه الصفحة ؟ ", "Close or Not", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
